...
void ordena_contagem_menores(lista *l){
    ...
    for(i = 0; i < l->tamanho; i++)  //custo: maior - menor + 1 (nesse caso menor = 0)
        l->contagens[i] = 0;
...
void ordena_counting_sort(lista *l){
    ...
    for(i = 0; i < maior + 1; i++)  //custo: maior - menor + 1 (nesse caso menor = 0)
        contagens[i] = 0;

    // faz contagem
    for(i = 0; i < l->tamanho; i++) // custo: n 
        contagens[(int) l->elementos[i]]++; 

    // monta vetor ordenado 
    j = 0;
    for(i = 0; i < maior + 1; i++) //custo mmaior - menor + 1 + n (+ n vem do while)
        // explicacao: https://www.youtube.com/watch?v=ChGPZdD6SI0 min 12
        while(contagens[i] > 0){
            l->elementos[j++] = i;
            contagens[i]--;
        }
    ...
    // custo toal: 2n + 2(maior - menor + 1)
    //      se n > maior - menor + 1, complexidade O(n)
    //      caso contrario, O(maior - menor + 1)
}

//* RADIXSORT

int retorna_digito(elem valor, int pos){
    
    for(int i = 0; i < pos; i++) // custo: O(n)
        valor /= 10;
    return valor % 10;
}

void ordena_radixsort(lista *l){ // custo total: O(n + m+ m(n*m/2 + m))
    // implementação mais simpels de fila, implementação dinamica com acesso sequencial
    
    //10 ponteiros para elemento, e cada ponteiro sera uma fila
    elem *filas[10];
    long tam_filas[10] = {0};

    long i, j, k, maior;
    int m, fila; // num de digitos do maior num

    // instancia filas 
    for(i = 0; i < 10; i++)
        filas[i] = malloc(sizeof(elem)*TAM);
    
    // encontra maior
    maior = l->elementos[10];
    for(i = 1; i < l->tamanho; i++) // custo: O(n)
        if(l->elementos[i] > maior)
            maior = l->elementos[i];

    // descobre quantos digitos o maior elemento possui
    m = 1;
    while(maior >= 10){ //O(m), m sendo o num de digitos do maior um
        maior /= 10;
        m++;
    }

    // faz m iteracoes
    for(i = 0; i < m; i++){ // custo: (mn)
        // insere nas filas
        for(j = 0; j < l->tamanho; j++){ // (n * m/2)
            fila = retorna_digito(l->elementos[j], i);
            filas[fila][tam_filas[fila]++] = l->elementos[j]; 
        }
    ...
    // Custo total O(n + m+ m(n*m/2 + m)), se m é pequeno comparado com n
    //O(mn) ou O(n)
    // pela limitação do "long m" nao vai ser possivel te rvalores grandes o bastante de m  
